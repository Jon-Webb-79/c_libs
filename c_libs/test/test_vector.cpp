...
TEST(test_initialize_array, init_data) {
	...
	dat_type dtype = INT;
	char name[6] = "array";
	Vector arr_test = init_array(dtype, indices, name);
...
TEST(test_find_indices, find_integer_indices) {
	...
	Vector p = find_int_array_indices(&arr_test, 6);
...
TEST(test_find_indices, find_float_indices) {
	...
	append_array(&arr_test, a, 7);
	float value = 6.2;
	Vector p = find_float_array_indices(&arr_test, value);
	EXPECT_EQ(int_array_val(&p, 0), 0);
	EXPECT_EQ(int_array_val(&p, 1), 3);
	EXPECT_EQ(int_array_val(&p, 2), 4);
	free_array(&arr_test);
...
TEST(test_find_indices, find_double_indices) {
	...
	append_array(&arr_test, a, 7);
	double value = 6.2;
	Vector p = find_double_array_indices(&arr_test, value);
	EXPECT_EQ(int_array_val(&p, 0), 0);
	EXPECT_EQ(int_array_val(&p, 1), 3);
	EXPECT_EQ(int_array_val(&p, 2), 4);
	free_array(&arr_test);
...
TEST(test_find_indices, find_char_indices) {
	...
	append_array(&arr_test, a, 7);
	char value = 'b';
	Vector p = find_char_array_indices(&arr_test, value);
...
TEST(test_replace_element, replace_double_element) {
	...
	append_array(&arr_test, a, 6);
	replace_double_array_element(&arr_test, 2.2, 3.4);
	EXPECT_DOUBLE_EQ(double_array_val(&arr_test, 0), 1.1);
	EXPECT_DOUBLE_EQ(double_array_val(&arr_test, 1), 3.4);
	EXPECT_DOUBLE_EQ(double_array_val(&arr_test, 2), 3.4);
	EXPECT_DOUBLE_EQ(double_array_val(&arr_test, 3), 3.4);
